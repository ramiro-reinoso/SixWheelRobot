...
public class DriveStraightTest extends AutonomousMode {
	...
	public DriveStraightTest (Robot r) {
		theRobot = r;
	}
...
//		theRobot.shifter.set(false);
        ...
        trajPointsRight.addTrajectoryPoints("rw_traj_DriveStraightTest_ticks.csv",1.0);
        trajPointsLeft = new ReadTrajectoryPoints();
        trajPointsLeft.addTrajectoryPoints("lw_traj_DriveStraightTest_ticks.csv",1.0);
...
//			System.out.println("PointsCount: " + pointsCount);
	...
	public void run() {
		...
		while(!Thread.interrupted() &&(!theRobot.rightTrans.checkMotionProfileStatus()) && !theRobot.leftTrans.checkMotionProfileStatus()) {
			autonomousPeriodic ();
			try {Thread.sleep(20);}
			catch (Exception e ) {	
			}
